...
test("browser capture tools reject main Session and can close their own Agent Window", async () => {
  const calls = [];
  ...
  assert.deepEqual(await close.execute({}, { threadKey: "capture:artifact-1", browserWorkflowId: "workflow-1" }), { closed: 1 });
