...
Ordinary DSH chat receives this Bridge core: `workflow.context`, `knowledge.search`, `knowledge.read_snippet`, `knowledge.fetch_url`, `today.read`, `capture.start`, `capture.status`, `capture.list_pending`, `jobs.list`, `jobs.submit` and `image.read`. `knowledge.fetch_url` is a governed read-only capability. `jobs.submit` remains the general-purpose chat write gateway. Goals, claims, evidence, settings and mechanical browser tools remain hidden Host capabilities, and the Host intersects requested names with the profile tool set; client-supplied `allowedTools`, profile, permission, authority, Owner or path fields cannot expand it. Capture Sessions use their separately issued browser allowlist.
...
- `ownerKey` is the isolation boundary for Goal/Job/Workflow/Note references and all durable clarification/recovery views. Read-side listings (`jobs.list`, `goals.list`) are Owner-scoped.
- Job ID 直达的 advice、approve、reject、cancel、retry 等操作必须在服务端重新校验 Owner；历史会话或旧微信命令不能提供隐式授权。
- The Project subsystem (server-owned `projectRef`, `/project` directive, `projects.*` tools) was removed on 2026-09-01 (product-slimming D6). Historical `ops/projects/*.md` records stay read-only; no Project authorization path remains.
