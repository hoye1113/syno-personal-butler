...
**文件名超长致收录提交失败 → 修复 + 恢复（2026-08-06）：** `workflow-b34a192f` 主人确认后执行阶段 `EXECUTION_FAILED`——`slug()` 截 60 + 8 位 hash = 69 > `validator.mjs` 文件名上限（`maxFilenameLength` 默认 50，无 contract 覆盖）。修复：`ingest-service.mjs` 新增 `noteFilenameBase` 为 hash 后缀预留 9 字符、base 截到 ≤41（commit `c5677af`，回归测试 `tests/knowledge-loop.test.mjs`）。恢复：该 workflow 已 `failed_terminal`（retry 拒收），改走外科式 `POST /api/syno/intake/{artifactId}/apply`（先就地修 artifact `proposal.suggestedPath` 为合规 49 字符名）+ keep-separate → job-20260806-b2e51e22 完成笔记写入、git 提交（`4ddcfef`）。「✅ 已收录」回执经 workflow-outbox 投递成功（outbox-296bac038387eb390d5d，2026-08-06T07:21:50Z delivered）。

**`#receive` 去重改动撤回（2026-08-06）：** 期间曾据「failed_terminal 卡死同源重收录」前提改 `#receive`（`related.find(stage!=="failed_terminal")`），核查发现**前提不成立且破坏测试 L83**（reported 终态会被误判 duplicate）——已 `git restore` 撤回、未提交、无行为变更。正确机制见 memory `syno-ingest-dedup-terminal-mechanism`：`findBySourceIdentityKey` 用 `includeTerminal:false`，同源**新消息**重发本就重建；唯同 messageId 重投被挡（防平台重投循环，故意设计）。核验：coordinator 测试 22/22、全量 `pnpm test` **719/719**、`pnpm verify` 通过（2026-08-06T08:05Z，host 停机状态下跑）；host 已 `windows:restart` 重拉、health ready。
