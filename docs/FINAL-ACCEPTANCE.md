...
当前结论：**全部必选验收项通过，可以完成最终外部切换。** 当前代码基线为 `0cc2669`：Windows Web 生命周期已纳入 Policy/Job 审计，真实卸载响应不中断 Host，launcher 从 2 个收敛到 0、重装后严格为 1，Today/微信设置完成桌面与移动复验；内部知识闭环、真实固定 Provider、微信、飞书消息与日历仍有既有直接证据。分支、原知识库、凭据与不 Push 边界持续满足。
...
| 最终备份、启动、回滚和切换 | 通过 | 真实状态归档 52 项、`credentialsIncluded=false`，清单校验、空目录 restore、二次覆盖拒绝和双渠道重启恢复通过；旧 `3c2b362` 归档保留为历史恢复点，当前分支另生成完整 Git bundle 并在最终交接记录 SHA-256；微信附件直发通过 |
