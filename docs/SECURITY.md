...
The supervised chat process is `dsh --profile syno web` (loopback). Capture remains jsonrpc. Credentials never appear in arguments, repository state or API output. Bridge tokens are not exposed on the DSH Web browser surface. Loopback `dsh web` (default `127.0.0.1:3088`) uses `approval: never` and `workspace-write` so WeChat and the browser share one session; any local process that can open that origin is a privileged agent for that session. Vault writes still go through `syno_*`.
