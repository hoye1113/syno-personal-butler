...
- `CognitiveRuntime` is the only model-loop seam: `run(request)`, `cancel(runId)`, `health()` and `capabilities()`. The only product implementation is `DeepSeekHarnessCognitiveRuntime` (`adapter: deepseek-harness-sdk`, v3). Production chat talks to a supervised `dsh --profile syno` Web process over loopback HTTP RPC plus one multiplexed WebSocket (`/api/remote.mux`; per-session `session/follow` logical streams carry durable events; 0.1.7 requires the launch-token cookie exchange first). Capture/ingest analysis uses the official DSH `sdk-minimal` profile over stdio JSON-RPC; its Syno overlay disables both persistent shell tools and adds only the Syno Tool Bridge. The explicit JSON-RPC chat fallback uses DSH's `sdk` profile. Tests and `SYNO_DSH_FAKE_AGENT` keep the JSON-RPC fake. The DSH clone at `SYNO_DSH_ROOT` must be built (`pnpm run build`); `dsh web` boots the stock `web` profile, not production `syno`.
...
- Syno deterministically selects one model attempt from the fixed DeepSeek chain (`deepseek-v4-flash-vision-exp` → `deepseek-v4-flash`). The Agent cannot select providers, models, permissions or approval levels, and a retry is forbidden after an irreversible effect. Harness chat may use sandboxed `workspace-write` tools (including official `web_search` / `web_fetch`) only inside an isolated profile directory under the Host local data root; capture/ingest analysis uses a second sidecar with no bash/fs/web. Repository `vault/` / `apps/` writes still go through `syno_*`, Policy and GitGuard. The primary DSH model is declared with text/image input modalities, but Syno's current attachment path still registers images and calls the Host `syno_image_read` tool (`mimo-v2.5-free`); direct DSH attachment admission is not yet part of this boundary. The production permission table contains only `workspace-write` + `never`; do not add `danger-full-access`.
- `DeepSeekHarnessSupervisor` owns the chat Web PID tree (or JSON-RPC process) and capture JSON-RPC process, isolated home/session/workspace roots, the Host-generated `syno` / `syno-lab` profiles (production `syno` links `@syno/dsh-plugin` into the profile `node_modules`), and the two SDK overlays (`syno-chat.cordis.yml`, `syno-capture.cordis.yml`). `SYNO_DSH_ROOT` is required; there is no hard-coded clone path. Production profile forbids marketplace `dsh plugin add`; `syno-lab` is the only place for `dsh-mnemon`, and its data is not canonical Syno memory. Loopback DSH Web is a privileged same-session shell (`approval: never`), not the control-plane UI. Pitfalls: `docs/OPERATIONS.md`.
...
- `SynoToolBridge` is the sole static MCP adapter at `POST /api/syno/bridge/mcp`. Its `exposedToolNames` seam and the shared `core` tool set expose `workflow.context`, `knowledge.search`, `knowledge.read_snippet`, `knowledge.fetch_url`, `today.read`, `capture.start`, `capture.status`, `capture.list_pending`, `jobs.list`, `jobs.submit` and `image.read` to ordinary DSH chat. URL fetching is a governed read-only capability. Hidden goal/claim/evidence/settings and mechanical browser tools remain in `ToolRegistry`; capture receives its explicit browser allowlist. `tools/list` filtering in the DSH plugin and `tools/call` filtering in the Harness runtime are both defense-in-depth. The Bridge never exposes arbitrary HTTP, filesystem, Shell or Git.
...
## Project subsystem (removed)

The Project-aware Knowledge MVP was physically removed on 2026-09-01 under product-slimming decision D6: `projects.*` tools, the `/project <projectRef>` directive, `ProjectService`, retrieval `PROJECT_BOOST` and active `projectRef` propagation no longer exist. Historical `ops/projects/*.md` records and the `projectRef`/`project_refs` fields on old Job/Workflow/Note records are retained read-only; readers tolerate and ignore them. The authoritative history is `docs/project-aware-knowledge-execution-plan.md`.
