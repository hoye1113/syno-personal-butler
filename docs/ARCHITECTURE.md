...
- DSH owns the conversational session, official compaction and same-session ordinary scheduling. Syno does not maintain a second active transcript or a second general-purpose memory provider. With the current Harness checkout, `@deepseek-ai/dsh-base` supplies the official compaction backend, `@deepseek-ai/dsh-web-app` supplies the Web bundle, and `@deepseek-ai/dsh-schedule` is mounted as an official function plugin by the `@syno/dsh-plugin` overlay; the production `syno` profile pins those two bundles plus `@syno/dsh-plugin`. `syno-lab` keeps the base/web bundle and preserves DSH CLI-installed experimental dependencies without mounting the Syno Bridge.
...
- `SynoToolBridge` is the sole static MCP adapter at `POST /api/syno/bridge/mcp`. Its `exposedToolNames` seam and the shared `core` tool set expose only `workflow.context`, `knowledge.search`, `knowledge.read_snippet`, `today.read`, `capture.start`, `capture.status`, `capture.list_pending`, `jobs.list`, `jobs.submit` and `image.read` to ordinary DSH chat. Hidden learning/goal/claim/evidence/settings and mechanical browser tools remain in `ToolRegistry`; capture receives its explicit browser allowlist. `tools/list` filtering in the DSH plugin and `tools/call` filtering in the Harness runtime are both defense-in-depth. The Bridge never exposes arbitrary HTTP, filesystem, Shell or Git.
