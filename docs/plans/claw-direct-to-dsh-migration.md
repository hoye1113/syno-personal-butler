...
- B5 切换与删除（**切换开关完成 2026-09-24**）：`SYNO_CHAT_TOOLS=plugin` 时 bridge 插件静默退出、capabilities 注册 canonical `syno_*` 名（`canonicalToolName`）；默认仍为 bridge（生产不变）。生产 bundle 已挂载 `plugins/capabilities` 与 `plugins/channel-weixin` 行（门控默认关闭，未设 env 时行为不变）。双模式真实 DSH spike 均绿。**删除类操作待 Owner 验收后再执行（保留一个版本）**：删除 `syno-tool-bridge.mjs`、`syno-tool-bridge-plugin.mjs`、`/api/syno/bridge/mcp` 与 allowlist 对应项、`FALLBACK_TOOLS`、`SYNO_BRIDGE_CONTEXT_*` 及相关 journal/测试替身；`syno-tool-sets.mjs` 名单转为插件工具集定义；收录 sidecar 改挂 capabilities（workflow 授权经现有 WorkflowStore 解析）；接管时保留 owner/allowedTools 语义（通道会话映射）、`agentAdjustableBoundary`、`tool-result-serializer` 脱敏与 `toolSet: core|all` 过滤。
...
- C1 新插件 `@syno/dsh-channel-weixin`：迁移 `weixin-ilink.mjs`（客户端/适配器/QR/poller/quarantine/typing/`formatWx`）、`channel-delivery-outbox.mjs`、`OwnerChannelTargetStore`、`accepted-request-*`、`channel-continuation-store`、`channel-intent-router`、`pending-decision` 渠道部分与 `channel-conversation-handler` 的确定性路由（模型调用替换为 `followup`）。
  ...
  - **C1b 首片完成（2026-09-24）**：`ChannelConversationHandler` 全量确定性路由接回——新增 `channel-handler.mjs` 工厂（Runtime=in-process 会话适配、Core=`SynoCore` 门面、Ingest/Workflow/PendingDecision/Continuation/Inspiration 存储走 syno-core）；`syno-core.mjs` 迁入 core；weixin spike 验证「新对话」确定性回执 + 模型路径 `pong`。剩余：typing/ACK/Outbox 归位、维护页 C4、真实扫码与 C5 切换。
