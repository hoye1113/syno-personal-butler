...
- 微信 iLink 已完成扫码、Owner 绑定、4/4 连续回复、故障恢复和 durable seen ID 跨重启；真实 MD 已完成解密、隔离及修复后重放到 Artifact/Proposal。仍缺一次修复后由主人直接重发附件的回执确认。
- 飞书消息已完成 Owner 私聊、真实 ID 重放拒绝与重启恢复；飞书日历已完成 user 授权、主日历 CRUD、错误拒绝与恢复。
- `bc5937b` 的 fresh-clone、桌面/移动浏览器复验和真实状态备份恢复已通过；完成修复后附件直发确认、最终提交与该提交的 fresh-clone 前不宣称最终切换。
