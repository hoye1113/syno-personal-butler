...
source_path: "02-Resources/AI and Agents/Harness Context & Reliability/Astral创始人-智能体让PR成本归零 人类审查成瓶颈.md"
...
>
> **核心主张：智能体把提交一个看似合理 PR 的成本降到零，但人类审查成本不变——代码评审成为新瓶颈；应对之道是把自动化校验与 Codex Review 前置成 harness 不变量、用第一性原理引导系统级重设计，并坚持理解你提交的每一行。**

> 提交一个看起来合理的 PR 的成本已经降到了零，但审查它的成本却依然没变。
> ——Charlie Marsh
...
**核心判断：从核心简单、规则海量、极易扩展的 Linter 起步，而非必须完整才有用的类型检查器——用更小体量快速验证想法、把可用东西交到用户手里迭代，正是建立势头的关窍。**
...
**核心判断：开发者营销不是贬义——你只有 10 秒让人明白「这为什么与我有关」，要放无歧义的基准图与有力标语、坦诚不标题党；Rust 被低估的优势是 Cargo 开箱即用与内存安全，让高性能产品近乎零妥协。**
...
**核心判断：提交看似合理 PR 的成本已归零、评审把关成本不变——代码评审成为新瓶颈；AI 政策的核心不是反 AI，而是「你需要理解你粘贴或提交的内容」，更深的隐患是开源「贡献者成长契约」的失效。**
...
**核心判断：Rust 只是性能下限，架构创新才决定上限——uv 靠精心设计的缓存布局让重复安装近乎瞬间、用单个 U64 整数表示 90% 以上版本号；Ty 基于 Salsa 惰性依赖图做增量分析，扛住大型项目。**
...
**核心判断：Agent 擅长局部微优化、不擅系统级重设计（Ghostty 案例：LLM 优化快 10 倍、手写版快 100 倍）；对抗灰色地带代码靠前置自动化校验 + 默认 Codex Review + AGENTS.md 沉淀 + 亲自通读每一行。**
...
**核心判断：智能体时代做工程师比以往更难也更重要——新人易被 agent 生成的大量代码淹没而不懂；关键是亲手审查、用第一性原理引导，而不是把判断外包给模型。**
