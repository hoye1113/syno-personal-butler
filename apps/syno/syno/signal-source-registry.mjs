class SignalSourceRegistry {
  ...
  async collect({ now = new Date() } = {}) {
    const [claims, intake, outputs, maintenance] = await Promise.all([
      ...
      this.ingest?.pending?.() || [],
      this.outputs?.list?.({ status: "suggested" }) || [],
    ...
    return [
      ...
      ...outputs.map((item) => ({ id: `output-opportunity:${item.id}`, kind: "output-opportunity", title: `推进创作：${item.title}`, priority: item.priority || 70, ref: item })),
