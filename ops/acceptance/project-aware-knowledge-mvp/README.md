...
The latest evidence pair is `jsonrpc-20260826T153428Z.json` and `web-search-20260826T153523Z.json`. Each live case snapshots the actual external `deepseek-harness` checkout before and after execution and asserts that tracked, staged and untracked status entries are unchanged; the evidence records only the redacted `unchanged`, `clean` and `entryCount` summary. The two cases were run independently and both exited with code 0. The live test uses a temporary `.runtime/tests` fixture for Project, Job, Workflow, Proposal and Vault state. It does not modify the external `deepseek-harness` checkout.
